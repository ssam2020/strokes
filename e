...
class App(Tk):
    def __init__(self):
        ...
        self.inv_var = BooleanVar(value=False)  # Add inversion variable
        ...
        # Buttons
        Checkbutton(self, text="Invert", variable=self.inv_var).grid(row=len(TASKS), column=2, padx=10)
    ...
    def run_task(self, task_name, target_percent, target_folder, start_limit, end_limit):
        ...
        try:
            ...
            while True:
                if EXIT_FLAG:
                    print(f"Aborting {task_name}!")
                    pyautogui.press('esc', presses=2, interval=0.3)  # Cleanup
                    return
                    
                pyautogui.click(259, 47)
                
                # Break long sleep into smaller chunks with exit checks
                for _ in range(5):
                    if EXIT_FLAG: break
                    time.sleep(0.2)
    ...
    def move_processed_files(self, target_folder, task_name):
        """Move processed files to task-specific subfolder with sequential naming"""
        folder_name = task_name.split('.')[0]
        if self.inv_var.get():  # Add inversion check
            folder_name += "-inv"
        ...
        # Get and sort files by percentage
        files = [f for f in os.listdir(target_folder) if "fotosketcher" in f.lower() and "%" in f]
        ...
        print(f"Renamed and moved {len(files)} files to {dest_folder}")
